...
namespace prjQNAMAker01.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Index(string question)
        {
            ...
            string jsonResult = System.Text.Encoding.UTF8.GetString(result);
            QnAresponse qnAresponse = JsonConvert.DeserializeObject<QnAresponse>(jsonResult);

            string answer = qnAresponse.answers[0].answer.Replace("\n","<br>");

            if(answer.Contains("match found in KB"))
            {
                answer = "此問題無法回復，我們將回報主管";
